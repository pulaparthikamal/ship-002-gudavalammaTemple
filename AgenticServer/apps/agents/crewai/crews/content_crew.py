...
@register_crew("content")
class ContentCrew(BaseCrew):
    """
    Expert-Level Content Generation Crew with strict Model-Role Alignment.
    ...
    def setup_agents(self, inputs: dict[str, Any]) -> list[Agent]:
        # User-Specified Model Mapping
        m_reasoning = self._get_llm_for_model("gpt-oss:120b-cloud")
        m_human = self._get_llm_for_model("gemma4:31b-cloud")
        m_light = self._get_llm_for_model("ministral-3:14b-cloud")
        m_vision = self._get_llm_for_model("qwen3-vl:235b-instruct-cloud")
        ...
        # 4.1 Reddit Specialist
        reddit_strategist = Agent(
            role="Reddit Technical Specialist",
            goal="Generate a full community-oriented, high-signal Reddit post with TL;DR.",
            backstory=REDDIT_ADAPTER_BACKSTORY,
            llm=m_reasoning,
        )

        # 4.2 Twitter/X Thread Specialist
        ...
        return [
            ...
            linkedin_strategist, social_strategist, reddit_strategist,
            twitter_strategist, youtube_specialist,
            humanizer, sanitizer, designer, visual_validator
        ]
    ...
    def setup_tasks(self, agents: list[Agent], inputs: dict[str, Any]) -> list[Task]:
        (research_analyst, writer_agent, topic_validator, reviewer_agent,
         linkedin_strategist, social_strategist, reddit_strategist,
         twitter_strategist, youtube_specialist,
         humanizer, sanitizer, designer, visual_validator) = agents
        ...
        # 4.1 REDDIT — technical community post
        reddit_task = Task(
            description=(
                f"Generate a complete technical Reddit post for '{topic}'.\n"
                f"{audience_priority}"
                f"{novelty_follow_through}"
                f"{title_instruction}"
                "MANDATORY FORMAT:\n"
                "  - Title: Descriptive, under 300 chars, no clickbait\n"
                "  - Opening: Specific technical problem or observation (not a hook)\n"
                "  - Body: Numbered insights with bullet sub-points\n"
                "  - Community Question: A genuine open technical question\n"
                "  - TL;DR: 2-3 sentence summary at the END\n\n"
                "Tone: Practitioner-to-practitioner. No marketing language. "
                "Use Reddit markdown (**bold**, numbered lists). Minimum 250 words."
            ),
            expected_output=(
                "Full Reddit post with title, structured body (min 250 words), "
                "genuine community question, and TL;DR."
            ),
            agent=reddit_strategist,
            context=[review_task],
        )

        # 4.2 TWITTER/X THREAD
        ...
        human_task = Task(
            ...
            agent=humanizer,
            context=[linkedin_task, social_task, reddit_task, twitter_task, youtube_task],
        )
        ...
        return [
            ...
            linkedin_task, social_task, reddit_task, twitter_task, youtube_task,
