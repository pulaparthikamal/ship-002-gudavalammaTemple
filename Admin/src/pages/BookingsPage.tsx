import { useState } from 'react'
import { Button } from 'primereact/button'
import { Column } from 'primereact/column'
import { DataTable } from 'primereact/datatable'
import { Dropdown } from 'primereact/dropdown'
import { CheckCircle2 } from 'lucide-react'
import { PageHeader } from '@/components/ui/PageHeader'
import { ConfirmationDialog } from '@/components/ui/ConfirmationDialog'
import { YearFilter, yearToDateRange } from '@/components/ui/YearFilter'
import { useToast } from '@/hooks/useToast'
import { getApiErrorMessage } from '@/services/api/apiError'
import { useGetBookingLedgerQuery, useMarkBookingPaidMutation } from '@/services/api/endpoints/bookingApi'
import type { BookingLedgerEntry } from '@/services/api/endpoints/bookingApi'
import { useStaffTranslation } from '@/i18n/useTranslation'
import { formatDateTime } from '@/utils/date'

const ALL = '__all__'

type Option = { label: string; value: string }

function getOptionLabel(value: string, options: Option[]) {
  return options.find((option) => option.value === value)?.label ?? value
}

function getTypeOptions(t: (key: string) => string): Option[] {
  return [
    { label: t('All types'), value: ALL },
    { label: t('Darshan'), value: 'darshan' },
    { label: t('Seva'), value: 'seva' },
    { label: t('Accommodation'), value: 'accommodation' },
    { label: t('Prasadam'), value: 'prasadam' },
    { label: t('Donation'), value: 'donation' },
  ]
}

function getStatusOptions(t: (key: string) => string): Option[] {
  return [
    { label: t('All statuses'), value: ALL },
    { label: t('Confirmed'), value: 'confirmed' },
    { label: t('Pending'), value: 'pending' },
    { label: t('Completed'), value: 'completed' },
    { label: t('Cancelled'), value: 'cancelled' },
  ]
}

function getPaymentStatusOptions(t: (key: string) => string): Option[] {
  return [
    { label: t('All payment statuses'), value: ALL },
    { label: t('Pending'), value: 'pending' },
    { label: t('Paid'), value: 'paid' },
    { label: t('Waived'), value: 'waived' },
  ]
}

function bookerLabel(row: BookingLedgerEntry, t: (key: string, params?: Record<string, string | number>) => string) {
  if (row.devotee) return `${row.devotee.firstName} ${row.devotee.lastName}`.trim()
  if (row.guestName) return t('{{name}} (guest)', { name: row.guestName })
  return t('Guest')
}

function bookerContact(row: BookingLedgerEntry) {
  return row.devotee?.email ?? row.guestEmail ?? row.devotee?.phone ?? row.guestPhone ?? '-'
}

export function BookingsPage() {
  const { t } = useStaffTranslation()
  const [page, setPage] = useState(1)
  const [rowsPerPage, setRowsPerPage] = useState(20)
  const [sortField, setSortField] = useState('date')
  const [direction, setDirection] = useState<'asc' | 'desc'>('desc')
  const [year, setYear] = useState<number | null>(null)
  const [type, setType] = useState<string>(ALL)
  const [status, setStatus] = useState<string>(ALL)
  const [paymentStatus, setPaymentStatus] = useState<string>(ALL)
  const [markPaidTarget, setMarkPaidTarget] = useState<BookingLedgerEntry | null>(null)
  const { showToast } = useToast()

  const typeOptions = getTypeOptions(t)
  const statusOptions = getStatusOptions(t)
  const paymentStatusOptions = getPaymentStatusOptions(t)

  const { from, to } = yearToDateRange(year)

  const ledgerQuery = useGetBookingLedgerQuery({
    page,
    limit: rowsPerPage,
    sortfield: sortField,
    direction,
    criteria: [],
    type: type === ALL ? undefined : type,
    status: status === ALL ? undefined : status,
    paymentStatus: paymentStatus === ALL ? undefined : paymentStatus,
    from,
    to,
  })
  const [markBookingPaid, { isLoading: isMarkingPaid }] = useMarkBookingPaidMutation()

  const rows = ledgerQuery.data?.data ?? []
  const totalRecords = ledgerQuery.data?.total ?? 0

  const handleMarkPaid = async () => {
    if (!markPaidTarget) return
    try {
      await markBookingPaid({ id: markPaidTarget._id }).unwrap()
      showToast({ severity: 'success', summary: t('Marked as paid') })
    } catch (error) {
      showToast({ severity: 'error', summary: t('Could not mark as paid'), detail: getApiErrorMessage(error) })
    } finally {
      setMarkPaidTarget(null)
    }
  }

  const referenceSuffix = markPaidTarget?.paymentReference
    ? ` ${t('(UPI reference: {{reference}})', { reference: markPaidTarget.paymentReference })}`
    : ''

  return (
    <div className="temple-scope w-full space-y-4">
      <PageHeader
        eyebrow={t('Operations')}
        title={t('Bookings')}
        description={t(
          'Unified ledger of every darshan, seva, accommodation, prasadam, and donation booking — filterable by year, type, and status.',
        )}
      />

      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">{t('Year')}</label>
          <YearFilter value={year} onChange={(value) => { setYear(value); setPage(1) }} />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">{t('Type')}</label>
          <Dropdown
            value={type}
            options={typeOptions}
            onChange={(e) => { setType(e.value); setPage(1) }}
            className="w-48"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">{t('Status')}</label>
          <Dropdown
            value={status}
            options={statusOptions}
            onChange={(e) => { setStatus(e.value); setPage(1) }}
            className="w-48"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">{t('Payment')}</label>
          <Dropdown
            value={paymentStatus}
            options={paymentStatusOptions}
            onChange={(e) => { setPaymentStatus(e.value); setPage(1) }}
            className="w-56"
          />
        </div>
      </div>

      <DataTable
        value={rows}
        loading={ledgerQuery.isLoading || ledgerQuery.isFetching}
        emptyMessage={t('No bookings found for the selected filters.')}
        lazy
        paginator
        first={(page - 1) * rowsPerPage}
        rows={rowsPerPage}
        totalRecords={totalRecords}
        onPage={(e) => { setPage(Math.floor((e.first ?? 0) / (e.rows ?? rowsPerPage)) + 1); setRowsPerPage(e.rows ?? rowsPerPage) }}
        sortField={sortField}
        sortOrder={direction === 'asc' ? 1 : -1}
        onSort={(e) => { setSortField(e.sortField); setDirection(e.sortOrder === 1 ? 'asc' : 'desc') }}
        className="text-sm"
      >
        <Column field="date" header={t('Date')} sortable body={(row: BookingLedgerEntry) => formatDateTime(row.date)} />
        <Column field="type" header={t('Type')} sortable body={(row: BookingLedgerEntry) => getOptionLabel(row.type, typeOptions)} />
        <Column field="title" header={t('Title')} />
        <Column header={t('Booked By')} body={(row: BookingLedgerEntry) => bookerLabel(row, t)} />
        <Column header={t('Contact')} body={(row: BookingLedgerEntry) => bookerContact(row)} />
        <Column field="amount" header={t('Amount')} sortable body={(row: BookingLedgerEntry) => `₹${row.amount.toLocaleString('en-IN')}`} />
        <Column field="paymentReference" header={t('UPI Reference')} body={(row: BookingLedgerEntry) => row.paymentReference || '-'} />
        <Column field="paymentStatus" header={t('Payment')} sortable body={(row: BookingLedgerEntry) => getOptionLabel(row.paymentStatus, paymentStatusOptions)} />
        <Column field="status" header={t('Status')} sortable body={(row: BookingLedgerEntry) => getOptionLabel(row.status, statusOptions)} />
        <Column
          header={t('Actions')}
          body={(row: BookingLedgerEntry) =>
            row.paymentStatus !== 'paid' ? (
              <Button
                type="button"
                text
                size="small"
                icon={<CheckCircle2 className="h-4 w-4" aria-hidden="true" />}
                label={t('Mark as paid')}
                onClick={() => setMarkPaidTarget(row)}
              />
            ) : null
          }
        />
      </DataTable>

      <ConfirmationDialog
        open={markPaidTarget !== null}
        title={t('Mark this booking as paid?')}
        message={
          markPaidTarget
            ? t('This confirms {{amount}} for "{{title}}" has been received{{referenceSuffix}}.', {
                amount: `₹${markPaidTarget.amount.toLocaleString('en-IN')}`,
                title: markPaidTarget.title,
                referenceSuffix,
              })
            : ''
        }
        confirmLabel={t('Mark as paid')}
        confirmLoading={isMarkingPaid}
        onConfirm={() => void handleMarkPaid()}
        onClose={() => setMarkPaidTarget(null)}
      />
    </div>
  )
}
