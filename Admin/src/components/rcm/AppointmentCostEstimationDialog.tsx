...
import { useEstimateForAppointmentMutation } from '@/services/api/endpoints/claimPredictionsApi'
import { useUpdateAppointmentMutation } from '@/services/api/endpoints/appointmentsApi'
import { getApiErrorMessage } from '@/services/api/apiError'
import type { Appointment } from '@/types/appointment'
import { Brain, DollarSign, ShieldCheck, TrendingUp, Check, Info } from 'lucide-react'

interface AppointmentCostEstimationDialogProps {
  visible: boolean
  onHide: () => void
  appointment: Appointment
}

export function AppointmentCostEstimationDialog({ visible, onHide, appointment }: AppointmentCostEstimationDialogProps) {
  const [estimate, { data: predictions, isLoading, error }] = useEstimateForAppointmentMutation()
  const [updateAppointment, { isLoading: isUpdating }] = useUpdateAppointmentMutation()

  useEffect(() => {
    if (visible && appointment._id) {
      estimate(appointment._id)
    }
  }, [visible, appointment._id, estimate])

  const handleApply = async () => {
    if (!predictions || predictions.length === 0) return

    const totalPatientResponsibility = predictions.reduce(
      (sum, p) => sum + (p.predictedPatientResponsibility || 0),
      0
    )
    ...
    try {
      await updateAppointment({
        ...
        data: {
          estimate: {
            ...
            estimatedPatientResponsibility: totalPatientResponsibility,
          },
      ...
      console.error('Failed to update appointment estimate:', err)
    }
  }
      ...
      <div className="space-y-4">
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-12 gap-4">
            ...
            <p className="text-sm font-medium text-[var(--color-text-muted)] animate-pulse">Resolving suggested procedures, contract rates, and eligibility benefits...</p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-lg border border-[var(--color-border)]">
              <div className="flex items-center justify-between gap-3 border-b border-[var(--color-border)] bg-[var(--color-surface-muted)] px-4 py-2">
                <h4 className="text-xs font-bold uppercase text-[var(--color-text-muted)]">Suggested Procedures</h4>
                <div className="flex items-center gap-2">
            ...
            <article className="rounded-lg border border-[var(--color-warning-text)]/30 bg-[var(--color-warning-soft)] p-4">
              <div className="flex gap-3">
                ...
                <div className="text-xs text-[var(--color-warning-text)] leading-relaxed">
                  <strong>Notice:</strong> Suggested procedures can change after clinical documentation. Amounts use matched payer fee schedules and eligibility benefits first, with historical fallback only when real-time data is missing.
                </div>
              </div>
            </article>
          </div>
        ...
        {!isLoading && predictions && predictions.length === 0 && (
          <div className="text-center py-8">
            <p className="text-sm text-[var(--color-text-muted)]">No procedure codes could be suggested for this appointment reason.</p>
          </div>
        )}
      </div>
    </Dialog>
