 ...
 * falling back to the bundled default when none has been uploaded yet, or
 * when the uploaded file fails to load (e.g. no longer on disk).
...
export function TempleArtwork() {
  ...
  const src =
    templeProfile?.deityImageUrl && !loadFailed ? resolveApiAssetUrl(templeProfile.deityImageUrl) : gudavalammaDeviImage

  return (
    <div className="temple-login-art">
      <img className="temple-login-art-image" src={src} alt="" onError={() => setLoadFailed(true)} />
    </div>
  )
}
