...
export const referralService = {
  async create(data: any, locale: string, createdBy: string) {
    const item = await Referral.create({
      ...data,
  ...
  async getById(id: string, locale: string) {
    const item = await Referral.findOne({ _id: id, isDeleted: false });

    if (!item) {
      throw new AppError(t('referral.notFound', {}, locale), HTTP_STATUS.NOT_FOUND);
    }
  ...
  async softDelete(id: string, locale: string, updatedBy: string) {
    const item = await Referral.findOneAndUpdate(
