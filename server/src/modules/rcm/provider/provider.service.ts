...
export const providerService = {
  ...
  async getById(id: string, locale: string) {
    const item = await Provider.findOne({ _id: id, isDeleted: false });

    if (!item) {
      throw new AppError(t('provider.notFound', {}, locale), HTTP_STATUS.NOT_FOUND);
    }
  ...
  async softDelete(id: string, locale: string, updatedBy: string) {
    const item = await Provider.findOneAndUpdate(
